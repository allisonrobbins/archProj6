...
team_t team = {
    ...
    "Allison Robbins",     /* First member full name */
    "allisonrobbins@gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
  for (i = 0; i < dim; i+=4){
    for (j = 0; j < dim; j+=4){
      dst[RIDX(dim-1-j, i, dim)] = src[RIDX(i, j, dim)];//i,j
      dst[RIDX(dim-1-j, i+1, dim)] = src[RIDX(i+1, j, dim)];//i+1,j
      dst[RIDX(dim-1-j, i+2, dim)] = src[RIDX(i+2, j, dim)];//i+2,j
      dst[RIDX(dim-1-j, i+3, dim)] = src[RIDX(i+3, j, dim)];//i+3,j
      dst[RIDX(dim-1-(j+1), i, dim)] = src[RIDX(i, j+1, dim)];//i,j+1
      dst[RIDX(dim-1-(j+1), i+1, dim)] = src[RIDX(i+1, j+1, dim)];//i+1,j+1
      dst[RIDX(dim-1-(j+1), i+2, dim)] = src[RIDX(i+2, j+1, dim)];//i+2,j+1
      dst[RIDX(dim-1-(j+1), i+3, dim)] = src[RIDX(i+3, j+1, dim)];//i+3,j+1
      dst[RIDX(dim-1-(j+2), i, dim)] = src[RIDX(i, j+2, dim)];//i,j+2
      dst[RIDX(dim-1-(j+2), i+1, dim)] = src[RIDX(i+1, j+2, dim)];//i+1,j+2
      dst[RIDX(dim-1-(j+2), i+2, dim)] = src[RIDX(i+2, j+2, dim)];//i+2,j+2
      dst[RIDX(dim-1-(j+2), i+3, dim)] = src[RIDX(i+3, j+2, dim)];//i+3,j+2
      dst[RIDX(dim-1-(j+3), i, dim)] = src[RIDX(i, j+3, dim)];//i,j+3
      dst[RIDX(dim-1-(j+3), i+1, dim)] = src[RIDX(i+1, j+3, dim)];//i+1,j+3
      dst[RIDX(dim-1-(j+3), i+2, dim)] = src[RIDX(i+2, j+3, dim)];//i+2,j+3
      dst[RIDX(dim-1-(j+3), i+3, dim)] = src[RIDX(i+3, j+3, dim)];//i+3,j+3
    }
  }
